...
- **周波数領域随伴**: 目的関数が周波数領域量（S11・流束・遠方界）のみに依存する場合、`simulate_tm_freq` は**順方向シミュレーション2回**で勾配を計算 — タイムテープ不要、residual は O(設計セル×周波数)。完全AD を正解とした検証でコサイン類似度 1.000000（方向微分誤差 3×10⁻⁵）。
- **融合 Rust カーネル（オプション）**: 2D タイムループをキャッシュフレンドリーなネイティブカーネルにコンパイル（初回利用時に cargo build、Rust 無し環境は自動フォールバック）。M1 Pro 実測で 1024² float32 **5,040 Mcell-steps/s = XLA CPU 比 8.4倍**。周波数領域随伴は順方向実行のみで勾配が出るため、**勾配計算も同じ倍率で高速化**されます。`scripts/benchmark.py --backend native` で再現可能（jax-metal は未保守のため Mac は CPU バックエンドが正式サポート）。
...
- [x] 周波数領域随伴（勾配＝順方向2回、タイムテープ不要）と融合 Rust CPU カーネル（Apple Silicon で8.4倍）
- [ ] openEMS クロスチェック参照データ、PCB 製造 + NanoVNA 実測キャンペーン
- [ ] 周波数領域随伴と融合カーネルの 3D 展開
