...
`web/` にインタラクティブな three.js ビューア（Vite + TypeScript）があります: トポロジー最適化のフレーム再生、**Rust→wasm カーネルによるブラウザ内ライブ 2D FDTD**（`web/wasm-kernel/`、26 kB — ソース移動・銅の描画・ダブルスリット/放物面ミラーのシーン）、回転できる 3D 遠方界ローブ、gradenna vs openEMS の S11 重ね描き。

```bash
cd web/app && npm install && npm run dev   # データは生成済みでコミットされています
```

データの再生成は `scripts/export_viz.py`。wasm のビルドと GitHub Pages へのデプロイは `web/app/README.md` を参照。
