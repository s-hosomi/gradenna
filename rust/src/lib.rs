...
mod lib3d;
// Worker-pool primitives shared by both kernels (spin barrier, shared pointer,
// row partition, thread-count heuristic).
mod pool;

use pool::{choose_threads, row_block, Shared, SpinBarrier, ThreadTuning};
...
impl Slab {
    fn new(n: usize, npml: usize) -> Self {
        Slab { lo0: 0, lo1: npml, hi0: n - npml }
    }
}

/// 2D thread tuning, measured on an M1 Pro (do not retune): 2 threads below
/// ~128^2, 4 below ~192^2, all 6 P-cores at 256^2 and up.
const TUNING_2D: ThreadTuning = ThreadTuning { small_below: 16 * 1024, medium_below: 64 * 1024 };
...
macro_rules! make_kernel {
    ($scalar:ty, $run_name:ident) => {
        ...
        pub unsafe extern "C" fn $run_name(
            ...
            let cells = nx * ny;
            let nthreads = choose_threads(cells, nx, TUNING_2D);
